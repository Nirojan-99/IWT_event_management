...
<div class="signup">
	<div class="contain">
		<h1>SIGN UP <span class="heading alignP">as employee</span></h1>
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" id="sign">
		<div>
		...
		<div>
			<i class="fas fa-user"></i>
			<input type="text" placeholder="Father's Name" name="PFname" required>
		</div>
		<div>
		...
		<div>
			...
			<input type="mail" placeholder="E-mail" name="Pmail" required pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$">
		</div>
		<div>
			<i class="fas fa-phone"></i>
			<input type="text" placeholder="Mobile Number" name="PMnum" required pattern="[0-9]{10}">
		</div>
		<div>
			<i class="material-icons" >location_on</i>
			<textarea  rows="4" cols="52" placeholder="  Address..." name="Paddr"  ></textarea>
		</div>
		<div>
			<i class="fas fa-key"></i>
			<input type="password"  placeholder="Password" name="Ppass" id="pw1" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}">
		</div>
		<div>
			<i class="fas fa-key"></i>
			<input type="password"  placeholder="Confirm Password" id="pw2" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}">
		</div>
		<input type="submit" name="employeesign" id="submit" value="Sign Up    " id="submit" onclick="passwordchk()" disabled>
	</form>
	</div>
</div>

...
<!-- body section-->
		...
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="Cpass">
			<div id="Cpass">
				<p class="headingwind">Change Password</p>
				<input type="Password" name="passE" id="passE" placeholder="Current Password" required ><br>
				<i class="fas fa-eye" id="passEI" onclick="showEP();"  aria-hidden="true" ></i>
				...
				<input type="Password" name="p" id="passNewR" placeholder="Repeat Password" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" >
				<i class="fas fa-eye" id="passEI3" onclick="showEPNR();" aria-hidden="true" ></i>
				<input type="submit" name="changePass" id="changePass" class="editWB" onclick="passwordE()" >
			</div>
		</form>
		...
		if (mysqli_affected_rows($conn) > 0) {
			$sql="SELECT password FROM employee WHERE mail like '$mail'";

			if ($result = mysqli_query($conn, $sql)) {
		  while ($obj = mysqli_fetch_object($result)) {
		  	
		    if ($obj->password == $pass) {
		    	$sql="UPDATE employee SET password = '$passN' WHERE mail LIKE  '$mail'";
		    	mysqli_query($conn, $sql);
		    	$_SESSION['passP']=$passN;
		...
		$sql="SELECT password FROM employer WHERE email like '$mail'";

			if ($result = mysqli_query($conn, $sql)) {
		  while ($obj = mysqli_fetch_object($result)) {
		  	
		    if ($obj->password == $pass) {
		    	$sql="UPDATE employer SET password = '$passN' WHERE email LIKE  '$mail'";
		    	mysqli_query($conn, $sql);
		    	$_SESSION['passP']=$passN;
